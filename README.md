# Heavily WIP lot of checking so it takes a while
# Awesome-CustomRoms
This is a list of **active** custom roms based of other lists. This is not a list of gsis for that you should check a [list](https://github.com/TrebleDroid/treble_experimentations/wiki/Generic-System-Image-%28GSI%29-list) from TrebleDroid.
## What is considered active
An active custom rom needs to be either Android 15/16 or needs to have activity(such as blog posts or github activity with in this month). If you want to check inactive custom roms or what I have gone trough check Inactive.md.
## Sources
This is in no special order.
- [ImKKingshuk](https://github.com/ImKKingshuk/Awesome-CustomROM)
- [devadigax](https://github.com/devadigax/awesome-android-custom-rom)
- [MrHyperIon101](https://github.com/MrHyperIon101/Awesome-CustomRoms)
- [MordorMinion](https://github.com/MordorMinion/awesome-custom-roms)
- [wshamroukh](https://github.com/wshamroukh/Android_Custom_Rom_List)
- [musabcel](https://github.com/musabcel/android_rom_list)
- [pixelzdevelopment](https://github.com/PixelzDevelopment/Custom-ROM-List)
## The list of active roms
- [2by2 Project](https://project2by2.jp/)
- [Advanced XPerience OS](https://axpos.org/)
- [AfterlifeOS](https://afterlifeos.com/)
- [AICP](https://www.aicp-rom.com/)
- [AlphaDroid](https://alphadroid-project.github.io/)
- [BaikalOS](https://github.com/baikalos)
- [BlissRoms](https://blissroms.org/)
- [CalyxOS](https://calyxos.org/)
- [CherishOS](https://www.cherishos.com/)
- [CipherOS](https://cipheros.org.in/)
- [crDroid](https://crdroid.net/)
- [DerpFest](https://derpfest.org/)
- [DroidX-UI](https://github.com/DroidX-UI)
- [/e/OS](https://e.foundation/)
- [Ethereal OS](https://ethereal-os.github.io/index.html)
- [EuclidOS](https://github.com/euclidTeam)
- [Evolution X](https://evolution-x.org/)
- []()
- []()
- []()
- []()
- []()
- []()
- []()
- []()
